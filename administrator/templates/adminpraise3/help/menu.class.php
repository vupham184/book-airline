...
class AdminPraise3Menu extends JObject {
	...
	function renderComponentChildren($components, $showChildren) {
		...
		foreach ($components AS $i => $row) :
            ...
            if ($row->admin_menu_link) :
            	$ignore_first = false;
			    $html .= '<a href="'.$row->admin_menu_link.'" class="parent-link" title="'.JText::_($row->name).'"><div class="component-image"></div><span class="component-label">'.JText::_($row->name).'</span></a>';
			else : 
				$ignore_first = true;
					...
					$html .= "<li class=\"child first-child\"><span class=\"triangle\"></span><a href=\"index.php?option=$row->option\"><span class=\"component-child-image\"></span><span>".JText::_($row->name)."</span></a></li>";
      	        	foreach ($row->children AS $i2 => $child): 
						...
						$html .= "<li class=\"child\">";
      	        		...
      	        		$html .= "<a href='$child->admin_menu_link' title=\"".JText::_($child->name)."\"><span class=\"component-child-image\"></span><span>".JText::_($child->name)."</span></a></li>";
      	        		$k = 1 - $k;
      	        	endforeach;
					$html .= "</ul>";
				endif;
	...
	function renderComponentFooter($components) {
		...
		foreach ($components AS $i => $row) :
            ...
            if ($row->admin_menu_link) :
            	$ignore_first = false;
			    $html .= '<a href="'.$row->admin_menu_link.'" class="parent-link hasTip" title="'.JText::_($row->name).'::"><div class="component-image"></div><span class="component-label">'.JText::_($row->name).'</span></a>';
			else : 
				$ignore_first = true;
	...
	function renderMenusMenu($active) {
		...
		for($i=0; $i<count($menus); $i++) :
			$menu = $menus[$i];
			$url = 'index.php?option=com_menus&view=items&menutype='.$menu->menutype;
