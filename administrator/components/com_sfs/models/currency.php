 ...
 * Currency Model
 */
class SfsModelCurrency extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_sfs.currency.'.(int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_sfs.currency.'.(int) $record->id);
		}
		// Default
		else {
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Currency', $prefix = 'SfsTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_sfs.currency', 'currency', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id = (int) $this->getState('state.id')) {
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_sfs.edit.currency.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('currency.id') == 0) {
				...
				$data->set('country_id', JRequest::getInt('country_id', $app->getUserState('com_sfs.currencies.filter.country_id')));
			}
		}
	...
	protected function batchAccess($value, $pks)
	{
		...
		foreach ($pks as $pk) {
			if (!$user->authorise('core.edit', 'com_sfs.currency.'.$pk)) {
