 ...
 * Airport Model
 */
class SfsModelAirport extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_sfs.airport.'.(int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_sfs.airport.'.(int) $record->id);
		}
		// Default
		else {
	...
	{
		// Get the form.
		$form = $this->loadForm('com_sfs.airport', 'airport', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id = (int) $this->getState('airport.id')) {
	...
	protected function loadFormData() 
	{
		...
		if (empty($data)) {
			...
			if ($this->getState('airport.id') == 0) {
				$app = JFactory::getApplication();
				$data->set('country_id', JRequest::getInt('country_id', $app->getUserState('com_sfs.airports.filter.country_id')));
				$data->set('state_id', JRequest::getInt('state_id', $app->getUserState('com_sfs.airports.filter.state_id')));
			}
		}
	...
	protected function batchAccess($value, $pks)
	{
		...
		foreach ($pks as $pk) {
			if (!$user->authorise('core.edit', 'com_sfs.airport.'.$pk)) {
