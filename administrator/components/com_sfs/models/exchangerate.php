 ...
 * Exchangerate Model
 */
class SfsModelExchangerate extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_sfs.exchangerate.'.(int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_sfs.exchangerate.'.(int) $record->id);
		}
		// Default
		else {
	...
	{
		// Get the form.
		$form = $this->loadForm('com_sfs.exchangerate', 'exchangerate', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id = (int) $this->getState('exchangerate.id')) {
	...
	protected function loadFormData() 
	{
		...
		if (empty($data)) {
			...
			if ($this->getState('exchangerate.id') == 0) {
				$app = JFactory::getApplication();
				$data->set('country_id', JRequest::getInt('country_id', $app->getUserState('com_sfs.exchangerates.filter.country_id')));
				$data->set('state_id', JRequest::getInt('state_id', $app->getUserState('com_sfs.exchangerates.filter.state_id')));
			}
		}
	...
	protected function batchAccess($value, $pks)
	{
		...
		foreach ($pks as $pk) {
			if (!$user->authorise('core.edit', 'com_sfs.exchangerate.'.$pk)) {
