 ...
 * Exchangerate Model
 */
class SfsModelManagetemplatemobile extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_sfs.managetemplatemobile.'.(int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_sfs.managetemplatemobile.'.(int) $record->id);
		}
		// Default
		else {
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'managetemplatemobile', $prefix = 'SfsTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_sfs.managetemplatemobile', 'managetemplatemobile', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id = (int) $this->getState('managetemplatemobile.id')) {
	...
	protected function loadFormData() 
	{
		...
		if (empty($data)) {
			...
			if ($this->getState('managetemplatemobile.id') == 0) {
				$app = JFactory::getApplication();
				$data->set('country_id', JRequest::getInt('country_id', $app->getUserState('com_sfs.managetemplatemobiles.filter.country_id')));
				$data->set('state_id', JRequest::getInt('state_id', $app->getUserState('com_sfs.managetemplatemobiles.filter.state_id')));
			}
		}
	...
	protected function batchAccess($value, $pks)
	{
		...
		foreach ($pks as $pk) {
			if (!$user->authorise('core.edit', 'com_sfs.managetemplatemobile.'.$pk)) {
