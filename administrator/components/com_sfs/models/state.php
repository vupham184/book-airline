 ...
 * State Model
 */
class SfsModelState extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_sfs.state.'.(int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_sfs.state.'.(int) $record->id);
		}
		// Default
		else {
	...
	{
		// Get the form.
		$form = $this->loadForm('com_sfs.state', 'state', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData() 
	{
		...
		if (empty($data)) {
			...
			if ($this->getState('state.id') == 0) {
				...
				$data->set('country_id', JRequest::getInt('country_id', $app->getUserState('com_sfs.states.filter.country_id')));
			}
		}
	...
	protected function batchAccess($value, $pks)
	{
		...
		foreach ($pks as $pk) {
			if (!$user->authorise('core.edit', 'com_sfs.state.'.$pk)) {
