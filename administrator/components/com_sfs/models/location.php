...
class SfsModelLocation extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_sfs.location.'.(int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_sfs.location.'.(int) $record->id);
		}
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Location', $prefix = 'SfsTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_sfs.location', 'location', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_sfs.edit.location.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
	...
	{
		// Check that user has edit permission for every item being changed
		// Note that the entire batch operation fails if any item lacks edit permission
		$user	= JFactory::getUser();
		
		foreach ($pks as $pk) {
			if (!$user->authorise('core.edit', 'com_sfs.location.'.$pk)) {
				// Error since user cannot edit this item
