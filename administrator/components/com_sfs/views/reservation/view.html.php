...
class SfsViewReservation extends JView
{
	...
	protected $state;
	protected $list_voucher_code, $reservation_sub;

	public function display($tpl = null)
	{
		$this->state		= $this->get('State');
		$this->reservation	= $this->get('Reservation');
		$this->reservation_sub	= $this->get('Reservation_sub');
		$this->list_voucher_code = $this->get('ListVoucherCode');
	...
	{	
		...
		if( (int)$this->reservation->ws_room <= 0){
		$toolbar->appendButton('Link', 'edit', 'edit', 'javascript:void(0);');	
		}
		$toolbar->appendButton('Link', 'reminder', 'Reminder', 'javascript:void(0);');
