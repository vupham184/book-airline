...
<div class="width-40 fltrt">		
	<fieldset class="adminform">
		...
		<select name="airlines[]" multiple="multiple" style="width:50%; height:200px;">
			<option value="">Select Airlines</option>
			<?php 
			foreach ($this->airlines as $airline): 
				$airlineName = !empty($airline->airline_name) ? $airline->airline_name : $airline->company_name;
				$selected = '';
				if( $airline->id  == $this->taxi->airlines->airline_id)
				{
					$selected = 'selected';
				}
			?>
			<option value="<?php echo $airline->id;?>" <?php echo $selected;?>><?php echo $airlineName;?></option>	
			<?php endforeach;?>
		</select>
	</fieldset>	
	<fieldset class="adminform">
