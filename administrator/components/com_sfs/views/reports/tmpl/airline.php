...
<div class="width-100">
	<form action="<?php echo JRoute::_('index.php?option=com_sfs&view=reports&layout=airline&airline_id='.$this->airline->id)?>" method="post" id="airlineReportForm" name="airlineReportForm">
		<fieldset>
			<h1>Airline Report: <?php echo $this->airline->airline_name;?></h1>
			...
			<div>	
				<table>
					<tr>
						...
						<?php if( isset($this->airline->airlines) && count($this->airline->airlines) ) :?>
						<td style="padding-left:20px;">Select Airline: </td>
						<td>
							<select name="gh_airline" class="inputbox" style="width:150px;">	
								<option value="0">All</option>	
								<?php foreach ($this->airline->airlines as $air):?>
						...
						<td style="padding-left:20px;"><input type="submit" value="GENERATE" class="button" style="background:green;color:white;border:none;padding:2px"></td>
					</tr>
				</table>				 						
			</div>
		</fieldset>
		
		<input type="hidden" name="task" value="reports.airlineReport">
		<input type="hidden" name="airline_id" value="<?php echo $this->airline->id?>">
...
<div class="width-100 fltlft" id="airlineReportResult"></div>
