...
class SfsControllerImportcsv extends JController {
	...
	public function import()
	{
		...
		$uploadPath = JPATH_SITE.DS.'uploads'.DS.'csv'.DS.$fileName;
 		if(!JFile::upload($fileTemp, $uploadPath)) 
		{
			$this->setRedirect(JRoute::_('index.php?option=com_sfs&view=importcsv&error=1',false));
			//echo JText::_( 'ERROR MOVING FILE' );
				return;
		}
		
		if ( !file_exists( $uploadPath ) ) { //JPATH_BASE
			$csv_file =  JPATH_SITE.DS.'uploads'.DS.'csv'.DS.'test_CSV_v2_org.csv';
		}
		else {
			$csv_file =  $uploadPath;
		}
		...
		if (($handle = fopen($csv_file, "r")) !== FALSE) {
			...
			while (($row = fgetcsv($handle, 1000, ",")) !== FALSE) {
				if ( $r >= 1 ) { //bo nhung muc tieu de khg lay data
		...
		fclose($handle);
		$this->setRedirect(JRoute::_('index.php?option=com_sfs&view=importcsv&suss=1',false));
	}
}