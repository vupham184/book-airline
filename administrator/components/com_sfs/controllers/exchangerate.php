 ...
 */
class SfsControllerExchangerate extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_sfs.exchangerate.'.$recordId)) {
			return true;
		}

		// Fallback on edit.own.
		// First test if the permission is available.
		if ($user->authorise('core.edit.own', 'com_sfs.exchangerate.'.$recordId)) {
	...
	public function batch()
	{
		...
		$model	= $this->getModel('Exchangerate');
		$vars	= JRequest::getVar('batch', array(), 'post', 'array');
		$cid	= JRequest::getVar('cid', array(), 'post', 'array');

		// Preset the redirect
		$this->setRedirect('index.php?option=com_sfs&view=exchangerates');
