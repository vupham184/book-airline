...
class MenusHelper
{
	...
	public static function addSubmenu($vName)
	{
		JSubMenuHelper::addEntry(
			JText::_('COM_ADMINPRAISE_SUBMENU_MENUS'),
			'index.php?option=com_adminpraise&view=menus',
			$vName == 'menus'
		);
		JSubMenuHelper::addEntry(
			JText::_('COM_ADMINPRAISE_SUBMENU_ITEMS'),
			'index.php?option=com_adminpraise&view=items',
			$vName == 'items'
		);
        JSubMenuHelper::addEntry(
			JText::_('COM_ADMINPRAISE_SUBMENU_ADMINITEMS'),
			'index.php?option=com_adminpraise&view=adminitems',
			$vName == 'adminitems'
		);
	}
	...
	public static function getActions($parentId = 0)
	{
		...
		if (empty($parentId)) {
			$assetName = 'com_adminpraise';
		} else {
			$assetName = 'com_adminpraise.item.'.(int) $parentId;
		}
	...
	static public function getAssociations($pk)
	{
		...
		$query->innerJoin('#__associations as a ON a.id=m.id AND a.context='.$db->quote('com_adminpraise.item'));
