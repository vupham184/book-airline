 ...
 * @since		1.6
 */
class AdminpraiseModelAdminitem extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_ADMINPRAISE_ITEM';
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_adminpraise.item.'.(int) $record->id);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_adminpraise.item.'.(int) $record->id);
		}
	...
	function batch($commands, $pks, $contexts)
	{
		...
		if (empty($pks)) {
			$this->setError(JText::_('COM_ADMINPRAISE_NO_ITEM_SELECTED'));
			return false;
		}
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		$user	= JFactory::getUser();
		if (!$user->authorise('core.create', 'com_adminpraise')) {
			$this->setError(JText::_('COM_ADMINPRAISE_BATCH_MENU_ITEM_CANNOT_CREATE'));
			return false;
		}
	...
	protected function batchMove($value, $pks, $contexts)
	{
		...
		if (!$user->authorise('core.edit', 'com_adminpraise')) {
			$this->setError(JText::_('COM_ADMINPRAISE_BATCH_MENU_ITEM_CANNOT_EDIT'));
			return false;
		}
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_adminpraise.adminitem', 'adminitem', array('control' => 'jform', 'load_data' => $loadData), true);
		if (empty($form)) {
			return false;
		}
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'AdminpraiseMenu', $prefix = 'AdminpraiseTable', $config = array())
	{
	...
	protected function populateState()
	{
		...
		if (!($parentId = $app->getUserState('com_adminpraise.edit.item.parent_id'))) {
			$parentId = JRequest::getInt('parent_id');
		}
		$this->setState('item.parent_id', $parentId);

		if (!($type = $app->getUserState('com_adminpraise.edit.item.type'))){
		...
		if ($link = $app->getUserState('com_adminpraise.edit.item.link')) {
			$this->setState('item.link', $link);
		}

        $menutype = $app->getUserState('filter.menutype') ? $app->getUserState('filter.menutype') : 'main';
	...
	protected function preprocessForm(JForm $form, $data, $group = 'content')
	{
		...
		if (JFactory::getApplication()->get('menu_associations', 0)) {
			...
			$fieldset->addAttribute('description', 'COM_ADMINPRAISE_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
			foreach ($languages as $tag => $language)
			{
				if ($tag != $data['language']) {
					...
					$option = $field->addChild('option', 'COM_ADMINPRAISE_ITEM_FIELD_ASSOCIATION_NO_VALUE');
	...
	public function save($data)
	{
        ...
        $data['client_id'] = 1;
        $data['menutype'] = $app->getUserState('filter.menutype', 'main');
...
//
		...
		if (JFactory::getApplication()->get('menu_associations', 0)) {
			...
			if ($all_language && !empty($associations)) {
				JError::raiseNotice(403, JText::_('COM_ADMINPRAISE_ERROR_ALL_LANGUAGE_ASSOCIATED'));
			}
			...
			$query->where('context='.$db->quote('com_adminpraise.item'));
			...
			if (!$all_language && count($associations)>1) {
				...
				foreach ($associations as $tag=>$id) {
					$query->values($id.','.$db->quote('com_adminpraise.item').','.$db->quote($key));
				}
				$db->setQuery($query);
				$db->query();
				if ($error = $db->getErrorMsg()) {
		...
		foreach ($pks as $i => $pk)
		{
			if ($table->load($pk)) {
				...
				if (!array_key_exists($table->language, $languages)) {
					...
					if ($table->home == $value) {
						unset($pks[$i]);
						JError::raiseNotice(403, JText::_('COM_ADMINPRAISE_ERROR_ALREADY_HOME'));
					}
					else {
						$table->home = $value;
						if ($table->language == '*') {
					...
					if (!$onehome) {
						$onehome = true;
						JError::raiseNotice(403, JText::sprintf('COM_ADMINPRAISE_ERROR_ONE_HOME'));
					}
				}
			}
		}
