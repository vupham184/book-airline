...
class AdminPraise3Tools extends JObject {
	...
	private function _initVariables() {
		...
		$profileLink = "<a href=\"" . JURI::root() . "administrator/index.php?option=com_admin&task=profile.edit&id=" . $user->get('id') . "\" class=\"\" >". $user->get('username') ."</a>";
		
		$profileAvatar = "<a href=\"" . JURI::root() . "administrator/index.php?option=com_admin&task=profile.edit&id=" . $user->get('id') . "\" class=\"\" ><img src=\"". $avatarFile ."\" title=\"". JText::_( 'PROFILE' ) . " " . $user->get('username') ."\"/></a>";
		...
		$this->setParam('layout',    	JRequest::getCmd('layout'));
		...
		$this->setParam('showMyEditor',    		$document->params->get('showMyEditor', 0));
		...
		$this->setParam('showBottomComponentList',    $document->params->get('showBottomComponentList', 0));
		...
		$this->setParam('altToolbar',    	$document->params->get('altToolbar', 1));
		...
		$this->setParam('componentsAcl',    	$document->params->get('componentsAcl', 1));
	...
	public function checkAdminMenuHealth() {
		require_once(JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_adminpraise' 
				. DS . 'helpers' . DS . 'helper.php');
		AdminpraiseHelper::checkAdminMenuHealth();
	}
	
	/**
	 * This is very ugly, but there is no other way around this at the moment.
	 * If we don't set the user state of the modules filter client_id then
	 * the page to create new modules will show just the frontend modules.
	 */
	public function setUserStateOnDashboard() {
		$appl = JFactory::getApplication();
		
		$appl->setUserState('com_modules.modules.filter.client_id', 1);
	}
