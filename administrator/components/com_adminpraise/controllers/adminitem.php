 ...
 * @since		1.6
 */
class AdminpraiseControllerAdminitem extends JControllerForm
{
	...
	public function add()
	{
		...
		$context	= 'com_adminpraise.edit.adminitem';
		...
		if ($result) {
			...
			$this->setRedirect(JRoute::_('index.php?option=com_adminpraise&view=adminitem&menutype='.$menuType.$this->getRedirectToItemAppend(), false));
		}
	...
	public function batch($model)
	{
		...
		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_adminpraise&view=adminitems'.$this->getRedirectToListAppend(), false));
	...
	public function edit($key = null, $urlVar = null)
	{
		...
		if ($result) {
			...
			$app->setUserState('com_adminpraise.edit.adminitem.type',	null);
			$app->setUserState('com_adminpraise.edit.adminitem.link',	null);
		}
	...
	public function save($key = null, $urlVar = null)
	{
		...
		if ($data === false) {
			...
			$app->setUserState('com_adminpraise.edit.adminitem.data', $data);
		...
		$this->setMessage(JText::_('com_adminpraise_SAVE_SUCCESS'));
		...
		switch ($task) {
			case 'apply':
				...
				$app->setUserState('com_adminpraise.edit.adminitem.data',	null);
				$app->setUserState('com_adminpraise.edit.adminitem.type',	null);
				$app->setUserState('com_adminpraise.edit.adminitem.link',	null);
			...
			case 'save2new':
				...
				$app->setUserState('com_adminpraise.edit.adminitem.type',	null);
				$app->setUserState('com_adminpraise.edit.adminitem.link',	null);
				$app->setUserState('com_adminpraise.edit.adminitem.menutype',	$model->getState('item.menutype'));
	...
	function setType()
	{
		...
		$app->setUserState('com_adminpraise.edit.adminitem.type',	$title);
		if ($title == 'component') {
			if (isset($type->request)) {
				...
				$app->setUserState(
					'com_adminpraise.edit.item.link',
					'index.php?' . JURI::buildQuery((array)$type->request));
			}
		}
		// If the type is alias you just need the item id from the menu item referenced.
		elseif ($title == 'alias') {
			$app->setUserState('com_adminpraise.edit.adminitem.link', 'index.php?Itemid=');
		}

		unset($data['request']);
		$data['type'] = $title;
		if (JRequest::getCmd('fieldtype') == 'type') {
			$data['link'] = $app->getUserState('com_adminpraise.edit.adminitem.link');
		}
