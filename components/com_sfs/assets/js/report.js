...
function airlineReport(dateFrom,dateTo,ghAirline, uk_secret_key) {
	
	new Fx.Scroll(window).toElement($('your-report'));	
	
	$('total_booking_overview').empty().addClass('report-ajax-loading');
	...
	var reportRequests = {
		r0 : new Request.HTML({//totalbookingoverview
		       url: siteurl+'index.php?option=com_sfs&format=raw&task=report.totalbookingoverview&type=0' + uk_secret_key,
		       method: 'get',		  
		       update: $('total_booking_overview'),
		       onSuccess: function(txt){
		    	   $('total_booking_overview').removeClass('report-ajax-loading'); 
		       }
		}),
		r1 : new Request.HTML({
	...
	reportRequests.r0.send('date_from='+dateFrom+'&date_to='+dateTo+'&gh_airline='+ghAirline);	
