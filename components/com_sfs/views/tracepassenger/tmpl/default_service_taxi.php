...
<div class="info-service-header-taxi">
	<table class="tableInfo" cellpadding="0" cellspacing="0" border="0">
		<tr>
			<td width="40%">Taxi transfer</td>
			<td width="10%">
				<img src="<?php echo JURI::root().'media/media/images/VoucherNOK.png' ?>" class='taxi_status'>
			</td>	
			<td width="40%" class="taxi_name">Please choice Taxi</td>
		...
		<tr id="taxi-content">
			<td width="60%">
				<div class="taxi_services">
					<p class="push-left" >
						<input type="radio" name='taxi_option' class="taxi_option" data-option="option1" checked>From Airport To Hotel(default)
					</p>
					<p class="push-left" >
...
<script type="text/javascript">
	...
	var v_option_taxi ='<?php echo $this->item->option_taxi ?>';
	var v_taxi_id = '<?php echo $this->item->taxi_id ?>';
	var v_taxiFromAddress = "<?php echo $this->item->taxFromAndress ?>";
	var v_taxiToAddress = "<?php echo $this->item->taxToAddress ?>";
	var v_taxDistance = '<?php echo $this->item->taxDistance ?>';
	var v_taxTotalPrice = '<?php echo $this->item->taxTotalPrice ?>';
	var hotel_id = '<?php echo (int)$this->item->hotel_id ?>'; 
	var v_taxWayOption = "<?php echo $this->item->taxWayOption; ?>";
	var v_taxCpnName =  "<?php echo $this->item->taxCpnName; ?>";
	var not_click = true;
	var way = v_taxWayOption;
	var load = 1;
	jQuery.noConflict();
	jQuery(function($){
...
function autoLoad(data, data_option_taxi ){
	...
	switch(data_option_taxi){
		...
		initMap(way);
		break;
		default:$('.other-taxi-choice').hide(); break; 
	}
}
...
function switchTaxiCheck(){
	...
	if( not_click == true ) {
		...
		function displayPriceOptionOther(){
			$('.btnTaxiWay').off('click').on('click', function() {
				way = $(this).attr('value');
				initMap(way);
		...
		function saveTaxiInfo(distance,total_price,way){
			...
			var id_passengers = [];
			id_passengers.push($('#passenger_id').attr('value'));
			...
			.done(function(re_data) { console.log(re_data);
				if (re_data>0) {
