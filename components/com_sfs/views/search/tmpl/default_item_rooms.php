...
$room_book = json_decode(JRequest::getVar('room_book'));
$pass_issue_hotel = explode("_",JRequest::getVar('pass_issue_hotel'));
$air = SFactory::getAirline();

$symbol_Or_Code = !empty($this->item->currency_symbol) ? $this->item->currency_symbol : $this->item->currency_code;
...
$query = 'SELECT * FROM #__sfs_currency WHERE code = "'.trim((string)$air->currency_code).'"';
$db->setQuery($query);	
$code = $db->loadObject();	
...
<style type="text/css">
	...
	if( !empty($this->item->single_room_available) && (int)$this->item->single_room_available == 1 ) :?>
		...
		<tr class="room-item">
            ...
            <td class="result-row-room-price"><?php echo $symbol_Or_Code." ". number_format($price, 2, ".", "") . 
        			"<br /><span class='hoverRate' 
        			title='Test' style='font-size:12px; font-weight:400;'>".$air->currency_code.'&#160;'. number_format($prideUsd, 2, ".", "") . "</span>"; ?></td>
            <td class="result-row-room-total"><?php echo '<span class="price-search-room">'.$this->item->s_room_total.'</span>'; ?></td>
	        <td class="room-item-last-cell">
				<input type="text" name="s_room" onchange="roomS(<?php echo $this->item->s_room_total;?>)" value="<?php if(count($room_book)>0 && count($pass_issue_hotel)>0){ echo $room_book[1]; } else echo $needed;?>" size="1" class="smaller-size" />
	...
	<tr class="room-item">
        ...
        <td class="result-row-room-total"><?php echo '<span class="price-search-room">'.$this->item->sd_room_total.'</span>'; ?></td>
        <td class="room-item-last-cell">
	        <?php
            $sd_needed = 0;
	        ...
	        ?>
	        <input type="text" name="sd_room" onchange="roomSD(<?php echo $this->item->sd_room_total;?>)" value="<?php if(count($room_book)>0 && count($pass_issue_hotel)>0){echo $room_book[2];}else echo $sd_needed;?>" size="1" class="smaller-size" />
    ...
    <tr class="room-item">
        ...
        <td class="result-row-room-price"><?php echo '<span class="price-search-room">'.$symbol_Or_Code." ".number_format($price, 2, ".", "").'</span>'; ?></td>
        <td class="result-row-room-total"><?php echo '<span class="price-search-room">'.$this->item->sd_room_total.'</span>'; ?></td>
    ...
    <tr class="room-item">
        ...
        <td class="result-row-room-price"><?php echo $symbol_Or_Code." ". number_format($price, 2, ".", "") . 
        			"<br /><span class='hoverRate' title='Test' style='font-size:12px; font-weight:400;'>".$air->currency_code.'&#160;'. number_format($prideUsd, 2, ".", "") . "</span>"; ?></td>
        

        <td class="result-row-room-total"><?php echo '<span class="price-search-room">'.$this->item->t_room_total. "</span>"; ?></td> 
        ...
        <td class="room-item-last-cell" style="padding-right:5px;">
	        ...
	        ?>					       	
        	<input type="text" name="t_room" onchange="roomT(<?php echo $this->item->t_room_total;?>)" value="<?php if(count($room_book)>0 && count($pass_issue_hotel)>0){echo $room_book[3];} else  echo $t_needed;?>" size="1" class="smaller-size" />
    ...
    if( !empty($this->item->quad_room_available) && (int)$this->item->quad_room_available == 1 ) : ?>
		...
		<tr class="room-item">
	    	<td class="result-row-room-title">Q Room:</td>
       		<td class="result-row-room-price"><?php echo $symbol_Or_Code." ". number_format($price, 2, ".", "") . 
        			"<br /><span style='font-size:12px; font-weight:400;'> ".$air->currency_code.'&#160;'. number_format($prideUsd, 2, ".", "") . "</span>"; ?></td>
	        <td class="result-row-room-total">
	        	<?php echo '<span class="price-search-room">'.$this->item->q_room_total. "</span>"; ?>
	        </td>
	        <td class="room-item-last-cell" style="padding-right:5px;">		      
		        <input type="text" name="q_room" onchange="roomQ(<?php echo $this->item->q_room_total;?>)" value="<?php if(count($room_book)>0 && count($pass_issue_hotel)>0){echo $room_book[4];}else echo '0'; ?>" size="1" class="smaller-size" />				        
