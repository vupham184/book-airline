...
if($this->show_all == false){
    $limit_show = $airline->partner_limit_for_extra_search;
}
$stt = 1;
$tmpl=JRequest::getVar('tmpl');
$css_hidden='';
if($tmpl=='component'){
    $css_hidden='style="display:none;"';
}
foreach ($dataNew_ as $key => $item) :
    ...
    ?>
    <tbody action="<?php echo JRoute::_('index.php?option=com_sfs')?>" id="<?php echo $formName;?>" <?php echo $css_hidden; ?> name="<?php echo $formName;?>" method="post" class="id-filter sfs-form form-vertical <?php echo $formName?> hotel_id_<?php echo $item->hotel_id;?>">
            ...
            <div style="width: 200px; height: 280px; padding: 5px 20px 5px 10px; background-color: #DFEFFF; border-radius: 5px; position: relative; font-weight: bold">
                <table width="100%" cellpadding="0" cellspacing="0" class="room-table">
                    ...
                    <?php if(empty($item->wsData)) : ?>
                        ...
                        <?php if( (int)$item->distance > 0 && (int) $this->item->transport_included == 0) :?>
                            <tr>
                        ...
                        <?php endif;?>
                    <?php else:?>
                        <?php if( (int)$item->distance > 0 && (int)$this->item->transport_available == 0 && (int) $this->item->transport_included == 0) :?>
                            <tr>
                                <td>Estimated<br/>&nbsp;taxi cost<br/>(return trip)</td>
                                <td style="vertical-align: middle">
                                    <?php echo $item->currency_symbol?>
