...
<div class="htmlvoucher">
	...
	<div style="background-color:#fff; border:solid 10px #82ADF1; padding:10px;font-family: 'lucida grande', tahoma, verdana, arial, sans-serif">
		...
		<p>
			...
			<?php 
			if( count($this->individualVoucher->passengers) )
			{
				$this->voucher_names = implode(', ', $this->individualVoucher->passengers);
			} else {
				$this->voucher_names = '';
			}
			if( $this->voucher_names ) : ?>
				Voucher issued for: <?php echo $this->voucher_names; ?><br />
			<?php endif;?>	
			
			Hotel Voucher Code: <?php echo $this->individualVoucher->code; ?><br />
			Voucher <?php if(count($this->individualVoucher->passengers)) echo "for ".count($this->individualVoucher->passengers)." person(s)";?>  entitled for:<br />
			
			<?php									 		
	 		switch ( (int)$this->individualVoucher->room_type )
	 		{
	 			case 1:
	...
	<div style="background-color:#FFDE73; border:solid 10px #FFB700; padding:10px; margin-top:15px;font-family: 'lucida grande', tahoma, verdana, arial, sans-serif">
		<p style="font-weight: bold;margin-top:5px;">
			...
			<?php echo 'Voucher code: '.$this->individualVoucher->code?><br />
			<?php echo 'Voucher '.$this->individualVoucher->room_type.' person'?><br />	
			<?php if($this->voucher->return_flight_number) : ?>
