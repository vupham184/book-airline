...
<div class="report-chart-wrap">
	...
	<div style="text-align:right;">
		<a id="average-modal" style="cursor:pointer; color:#0066CC;">View Full Image</a>
	</div>
</div>
...
<div class="report-detail-wrap">
		...
		<table width="100%" cellspacing="0" cellpadding="0" border="0" class="report-detail-table">
			<tbody>
				<tr>
			        ...
			        <td class="tableheader" style="border-right: 1px solid #666;" nowrap="nowrap"><?php echo JText::_('COM_SFS_REPORT_ADR')?></td>
				</tr>
				<?php 			
				foreach ($this->items as $item) {					
					echo '<tr class="sectiontableentry1">';
					echo '<td width="100%">'.$item->name.'</td>';
					echo '<td style="border-right: 1px solid #666; text-align:center" nowrap="nowrap">'.number_format($item->rate_total / $item->booked_total,2).' '.$item->currency_code.'</td>';
					echo '</tr>';
				}
				?>										
			</tbody>
	...
	<div>
		<form name="exportAveragePrices" action="index.php" method="post">
			...
			<input type="hidden" name="task" value="report.exportAveragePrices" />
