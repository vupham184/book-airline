...
<style type="text/css">
#mainPopupReport{float: left; width:100%; font-size: 13px;}
...
.textFill{width:60px; height: 25px;}
.pleaseSearch{margin-top: 10px;float:left;font-size: 13px;}
</style>
...
var listArrMailBcc = [];
    ...
    jQuery(function($){
                ...
                success:function(data){    
                    ...
                    if(data != ""){
                        ...
                        if(data.passengerInfo.length){
                            var html_sub = 'Attention all concerned at '+data.dep+' <br />Due to '+data.irreg_reason+' (and '+data.passengerInfo[0].irreg_reason+') ' +flight_+ ' ' +date_+ ' Please note following';
                        }
        ...
        $("#sendmessage").on('click', function (){
            var listmail = listArrMail, arr_text = [];   
            var listmailbcc = listArrMailBcc;                     
            arr_text = [$(".fillText_1").val(),$(".fillText_2").val(),$(".fillText_3").val(),$(".fillText_4").val(),$(".fillText_5").val()];
            ...
            $.ajax({
                url:"<?php echo JURI::base().'index.php?option=com_sfs&task=communication.sendmessageovb'; ?>",
                type:"POST",  
                data: {"dateId": $("#maDate").val(), "text_1": $(".textaform_1").val(), "listmail": listmail, "subject": $('.subject_com').text(), "listmailbcc": listmailbcc, "body_": arr_text},              
                dataType: 'json',  
                success: function(data){
...
<div id="mainPopupReport">
    <form>
        <table cellpadding="0" cellspacing="0" border="0" style="float:left; width:100%;background:#E1F1F9; padding: 10px 10px 20px 10px;">
                    ...
                    <div class="infomail">
                        ...
                        <div class="subinfomail" style="margin-top: 10px;">
                            <span class="subject_com">
                                <!-- Attention all concerned at TXL <br />
                                Due to DLY (and MISC) AB1000 29 Jun please note following -->
                            </span>
                        </div>                        
                    </div>
            ...
            <tr>
                <td colspan="4">
                    <span class="pleaseSearch">
                       Please search for <input type="text" class="fillText_1 textFill" > volunteers. Voluntary denied boarding passengers will recieve <input type="text" class="fillText_2 textFill" > and be protected via <input type="text" class="fillText_3 textFill" > in case of involuntary denied boarding passengers will recieve <input type="text" class="fillText_4 textFill" > and will be reprotected via <input type="text" class="fillText_5 textFill" >                       
                    </span>
                </td>                
            </tr>
            ...
            <tr>
                <td colspan="4">
                    <textarea class="textaform_1" style="margin-top:20px;">
