...
class SfsControllerGhregister extends JController
{
	...
	public function saveAirline()
	{
		...
		$airlineData['iatacode'] = $post['iatacode'];
		...
		$link = JRoute::_('index.php?option=com_sfs&view=ghregister&layout=contacts&Itemid='.$post['Itemid'],false);
	...
	public function saveContacts()
	{
		...
		if ( count($contacts) ) {
			...
			{				
				...
				if($result) 
				{				
					$app->enqueueMessage('E-Mail: '.$contact['email'].' in use' , 'warning');
					$errorChk = true;	
				}
		...
		$session->clear('gh_additional_contact');		
		
		if( $errorChk ) {		
			$link = JRoute::_('index.php?option=com_sfs&view=ghregister&layout=contacts&Itemid='.JRequest::getInt('Itemid'),false);
												
		} else {
			$link = JRoute::_('index.php?option=com_sfs&view=ghregister&layout=confirm&Itemid='.JRequest::getInt('Itemid'),false);					
		}
		$this->setRedirect($link);									
	}
	...
	public function addContact()
	{
		...
		if( ! $result ) {
			...
			$link =  JRoute::_('index.php?option=com_sfs&view=ghregister&layout=addcontact&tmpl=component&Itemid='.JRequest::getInt('Itemid') );
			$this->setRedirect($link);	
		}			
	}*/
	...
	public function addContact()
	{
		...
		if( ! $result ) {
			...
			$aContacts = null;
			if( ! $session->has('gh_additional_contact') ) {
				$aContacts = array();
			} else {
				$aContacts = $session->get('gh_additional_contact');
			}		
			$randomKey = SfsHelper::createRandomPassword(9);
			$aContacts[$randomKey] = $contact;				
			$session->set('gh_additional_contact',$aContacts);				
	...
	public function confirm() 
	{
		...
		$model = $this->getModel('Ghregister','SfsModel');
		
		$result = $model->save();
		
		if( $result ) {
			$link = JRoute::_('index.php?option=com_sfs&view=ghregister&layout=thankyou&Itemid='.$post['Itemid'],false);
			$this->setRedirect($link);	
		} else {
			...
			$link = JRoute::_('index.php?option=com_sfs&view=ghregister&Itemid='.$post['Itemid'],false);
			$this->setRedirect($link);			
		}
