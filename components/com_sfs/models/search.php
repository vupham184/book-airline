...
//        $timezoneOffset = $time->format('P');
		...
		if($ws)
 		{
			...
			if($wsOnly==1){
				//$this->setState('filter.ws_id_list', '');					
				$this->setState('filter.ws_id_list', $wsIDs);	
			}else{
				//if(JRequest::getVar('wshoteladd')){
					//$wsIDs=explode("_",JRequest::getVar('wshoteladd'));					
				//	$this->setState('filter.ws_id_list',$wsIDs);
				//}else{
					$this->setState('filter.ws_id_list', '');		
				//}
		...
		if (empty($this->_data)) {			
			...
			if (count($rows)) {				
				...
				if (count($contractedRates)) {
					...
					foreach ($rows as & $row) {
						$maxRate = new stdClass();
						$maxRate->srate = "false"; $maxRate->sdrate = "false";
						$maxRate->trate = "false"; $maxRate->qrate = "false";

						$rate = $contractedRates[$row->hotel_id];
						
						if($rate->max_rate == ''){
							$rate->max_rate = '{"srate":"false","sdrate":"false","trate":"false","qrate":"false"}';
						}
						...
						if ($contractedRates[$row->hotel_id] && empty($contractedRates[$row->hotel_id]->ex_hotel_id)) {
							...
							if( empty($contractedRates[$row->hotel_id]->max_rate) ){
								$row->contracted_max_rate = $maxRate;
							}else{
							...
							$row->contracted_two_course_dinner = floatval($contractedRates[$row->hotel_id]->two_course_dinner);
							$row->contracted_three_course_dinner = floatval($contractedRates[$row->hotel_id]->three_course_dinner);
							//End lchung
						}
      ...
      //                       $currency = SfsWs::getCurrencyIdByCurrencyCode($airport_currency);
			...
			if($wsOnly==0){
				...
				foreach($this->_data as $d){
					$total_room += $d->sd_room_total;
				}
				
				$airline = SFactory::getAirline();	
				//get ws hotel
				if($total_room < $airline->partner_limit_for_extra_search){
						$query = $this->getListQueryWS($db);
						$db->setQuery($query);
						$rows_ws = $db->loadObjectList();
						if($rows_ws){
							$contractedRates = $this->getContractedRates();							
							$rooms  = JRequest::getInt('rooms');
							$session = JFactory::getSession();
							...
							$date_start = JRequest::getVar('date_start');
							$wsCacheOnly=false;
							$wsHotels_ = SfsWs::searchHotels($airline->airport_code, $date_start, $rooms, $roomRequest , 1, $wsCacheOnly);
							$wsHotelsMap_ = array();
							

							/* @var $obj Ws_Do_Search_Result */
							if($wsHotels_) {
								foreach($wsHotels_ as $obj) {								
									$wsHotelsMap_[$obj->PrimaryID] = $obj;
								}
							}							
							if (count($contractedRates)){
								//$wsHotelsMap = $this->getState('filter.ws_hotels_map');
								foreach ($rows_ws as & $row) {
									$maxRate = new stdClass();
									$maxRate->srate = "false"; $maxRate->sdrate = "false";
									$maxRate->trate = "false"; $maxRate->qrate = "false";
									...
									if ($contractedRates[$row->hotel_id] && empty($contractedRates[$row->hotel_id]->ex_hotel_id)) {
															...
															if ($row->ws_id && $wsHotelsMap_[$row->ws_id]) {
																$row->wsData = $wsHotelsMap_[$row->ws_id];
									                            include JPATH_COMPONENT . DIRECTORY_SEPARATOR . 'ws' . DIRECTORY_SEPARATOR . 'config.php';
									                            if(strpos($config['params']['url'], 'totalstaytestxml')){
									                                $row->name  = "Test " .  $row->name;
									                            }
						...
						if($rows_ws){
							$wshoteladd='';
							foreach($rows_ws as $r){
								$this->_data[]=$r;	
							}
							$this->setState('wshoteladd', 1);	
						}
	...
	protected function getListQuery( $db, $type = 'local' )
	{
		...
		//join to currency
		$query->select('c.symbol AS currency_symbol, c.code AS currency_code');
	...
	public function getContractedRates()
	{
		...
		$query->select('a.hotel_id,a.sd_rate,a.t_rate,a.s_rate,a.q_rate,a.breakfast,a.lunch,a.dinner,a.two_course_dinner,a.three_course_dinner,a.max_rate');
	...
	protected function getListQueryWS( $db, $type = 'local' )
	{
		$airline = SFactory::getAirline();
        $airline_current = SAirline::getInstance()->getCurrentAirport();
        $airport_current_id = $airline_current->id;
        ...
        $query->where('b.ws_id IS NOT NULL');	
		$query->where('((a.sd_room_total > 0 and sd_room_rate > 0) or (a.t_room_total > 0 and a.t_room_rate > 0) or (a.s_room_total > 0 and a.s_room_rate > 0) or (a.q_room_total  > 0 and a.q_room_rate > 0))');
		if($type == 'local'){
			$query->select('e.distance, e.distance_unit');
			$query->innerJoin('#__sfs_hotel_airports AS e ON e.hotel_id=b.id AND e.airport_id='.$airport_current_id);
			$query->select('ia.km_rate AS km_rate_ws, ia.starting_tariff AS starting_tariff_ws');
