...
class SfsModelAccountingreports extends JModel
{
	...
	public function getPassengers( $export_withID = NULL )
	{
		...
		$query->leftJoin('#__sfs_voucher_codes AS d ON d.id=b.voucher_id');
		...
		$query->leftJoin('#__sfs_airplusws_creditcard_detail AS car ON car.airplus_id=a.id ');
		...
		///$query->where('b.airport_id='.(int)$airline->airport_id." AND b.created_date >= ".$db->quote($filter_date)." AND b.created_date <=".$db->quote($filter_until_date));
		$query->where('b.airport_id='.(int)$airline->airport_id);
		
		//$query->where('a.airport_code='.$db->quote($airline->airport_code)." AND a.startdate >= ".$db->quote($filter_date)." AND a.startdate <=".$db->quote($filter_until_date));
		
		///$query->where('a.startdate >= '.$db->quote($filter_date).' AND a.expiredate <= '.$db->quote($filter_until_date));
		*/
		...
		$query->select('p.voucher_id, p.id AS passenger_id, p.first_name, p.last_name, p.phone_number ' );
		
        $query->from('#__sfs_trace_passengers AS p');

        $query->select('pas.block_code,pas.service_id,pas.price_per_person,SUBSTRING_INDEX(pas.block_code,"_",1) AS airport_code');
		$query->leftJoin('#__sfs_passenger_service AS pas ON pas.passenger_id = p.id');

		$query->select('fi.*');
		$query->leftJoin('#__sfs_flightinfo AS fi ON fi.fltref = p.fltref');

		$query->select('svc.icon_service');
		$query->leftJoin('#__sfs_services AS svc ON svc.id = pas.service_id');


		$query->select('pa.id, pa.airline_id, pa.startdate, pa.expiredate, pa.flight_number,
		...
		$query->select('e.status as reservations_status,e.sd_room,e.t_room,e.s_room,e.q_room, e.sd_rate, e.t_rate, e.s_rate, e.q_rate,IFNULL(e.url_code,p.pnr) AS url_code');
		$query->select('d.status, e.blockcode,SUBSTRING_INDEX(e.blockcode,"_",1) AS airport_code_hotel, d.code AS voucher_code,d.comment,d.breakfast,d.lunch,d.mealplan, d.flight_id');
...
//		$query->where('pa.airport_code='.$db->quote($airline->airport_code)." AND pa.startdate >= ".$db->quote($filter_date)." AND pa.startdate <=".$db->quote($filter_until_date));
		...
		foreach ( $rows as $vk => $v ) {
			...
			$v->service_desc = $service_desc;
			$v->supplier = $supplier;
			$v->ticket_number = '';	*/	
			/*
			if( $v->voucher_id == 0 )
				$rowsN['k' . $v->id] = $v;//airplus id
			else
				$rowsN['k' . $v->voucher_id] = $v;
			*/
		}
		return $rows;
	}
    ...
    function getPassengerService($passenger_id){
		$db = $this->getDbo();
    	$query = $db->getQuery(true);
    	$query->select('a.*,s.name_service');
    	$query->from('#__sfs_passenger_service AS a ');
    	$query->where('passenger_id="'.$passenger_id . '"');
    	$query->leftJoin('#__sfs_services AS s ON s.id = a.service_id');
    	$query->group('s.name_service');
    	$query->order('a.service_id ASC');
    	$db->setQuery($query);
