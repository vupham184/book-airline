...
class SfsModelAirlineregister extends JModel
{
	...
	{				
		...
		foreach ($contacts as $key => $contact)
		{
			...
			{							
				...
				$airlineAdminName = $contact['name'].' '.$contact['surname'];
				...
				$this->_adminEmail($airline['iatacode_id'],$airlineId);
		...
		foreach ($contacts as $contact)
		{
			...
			{
				$userId = $this->saveAccount($contact, $airlineAdminName);
	...
	private function saveAccount( $contact, $airlineAdminName = null )
	{
		...
		if( $contact['main_contact'] == 1 ) {
			...
			$db->setQuery('SELECT id FROM #__usergroups WHERE title='.$db->quote('Airline Staff'));
		}
	...
	private function _adminEmail($airlineName,$airlineId)
	{
		...
		$data['mailfrom']	= $config->get('mailfrom');
		$data['sitename']	= $config->get('sitename');
		$data['approveurl']	= JUri::base().'administrator/index.php?option=com_sfs&view=airline&layout=edit&id='.$airlineId;		
